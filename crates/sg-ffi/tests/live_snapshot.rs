...
fn fixture_config(port: u16, refresh_ms: u64) -> TargetConfig {
    TargetConfig {
        ...
        auth_kind: "key".into(),
        key_path: Some(
            concat!(env!("CARGO_MANIFEST_DIR"), "/../../fixtures/id_test").to_string(),
        ),
        secret: None,
...
fn a_ui_polling_snapshots_sees_a_host_come_online_and_fill_in() {
    ...
    assert_eq!(snapshot.state, ConnectionState::Online, "never came online: {:?}", snapshot.state);
    assert!(snapshot.distro.contains("Debian"), "distro was {:?}", snapshot.distro);
    assert!(snapshot.cpu_count >= 1);
    assert!(!snapshot.display_name.is_empty());
    assert!(snapshot.source_errors.is_empty(), "collector errors: {:?}", snapshot.source_errors);
    ...
    assert_eq!(metrics.first(), Some(&"cpu_usage"), "grid order changed: {metrics:?}");
    for expected in ["cpu_usage", "mem_usage", "disk_usage", "uptime", "net_rx", "net_tx"] {
        assert!(
...
fn history_accumulates_so_sparklines_have_something_to_draw() {
    ...
    let snapshot = poll_until(&core, &id, Duration::from_secs(25), |s| {
        s.gauges.iter().any(|g| g.metric == "cpu_usage" && g.history.len() >= 3)
    });
    ...
    assert!(cpu.history.len() >= 3, "history did not accumulate: {:?}", cpu.history);
    assert_eq!(cpu.history.last().copied(), Some(cpu.value), "the gauge shows the newest point");
    assert!(
...
fn round_trips_grow_by_one_per_refresh() {
    ...
    assert!(refreshes >= 2, "only {refreshes} refreshes in {elapsed:.1}s");
...
fn unrecoverable_failures_are_reported_and_not_retried() {
    ...
    match snapshot.state {
        ConnectionState::Failed { recoverable, .. } => {
            assert!(!recoverable, "auth failure marked retryable would loop forever");
        }
